...
FROM inquiry_log
WHERE ts >= ($1::date AT TIME ZONE 'Europe/Zurich') AND ts < ($2::date AT TIME ZONE 'Europe/Zurich') AND age_group IS NOT NULL
GROUP BY 1;
...
FROM inquiry_log
WHERE ts >= ($1::date AT TIME ZONE 'Europe/Zurich') AND ts < ($2::date AT TIME ZONE 'Europe/Zurich') AND current_insurer IS NOT NULL
GROUP BY 1 ORDER BY 2 DESC LIMIT 10;
...
FROM inquiry_log
WHERE ts >= ($1::date AT TIME ZONE 'Europe/Zurich') AND ts < ($2::date AT TIME ZONE 'Europe/Zurich') AND current_premium_band IS NOT NULL
GROUP BY 1;
```
