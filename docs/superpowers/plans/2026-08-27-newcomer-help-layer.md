...
5. **`HelpTip`'s trigger is a `<summary>` element** (implicit button role + expanded state, native toggle), not the `<button aria-expanded>` the spec/mockup literally describe. Functionally equivalent; chosen for the free open/close semantics.
